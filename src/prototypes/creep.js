...
Creep.prototype.scoutDestination = function scoutDestination(GOTO,direction) {
    ...
    //console.log('FIND DIRECTION', exit);
    var result = this.moveTo(this.pos.findClosestByRange(exit));
    return result;
}
...
Creep.prototype.getDepositContainers = function() {
    ...
    var deposits = this.room.find(FIND_STRUCTURES, {
        filter: (structure) => {
            return (structure.structureType == STRUCTURE_CONTAINER) && structure.store.getFreeCapacity(RESOURCE_ENERGY) > 200;
        }
    });	
	return deposits;
};
...
Creep.prototype.getBuildSpot = function(creep,theBuildObjectCenter,surroundingSpace) {
    //console.log('theBuildObjectCenter',JSON.stringify(theBuildObjectCenter));
